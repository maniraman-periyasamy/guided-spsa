...
from config_classification import classificationConfig
...
@hydra.main(config_path="conf", config_name="config_clas", version_base="1.2")
def main(cfg: classificationConfig) -> float:
    test_avg = 0.0
    ...
    for dataset_name in datasets:
        ...
        for rep in range(cfg.algorithm_params.repeats):
            ...
            if dataset_name == "iris":
                ...
                cfg.dataset_params.n_classes = 3
                cfg.dataset_params.dataset_size = 150
                observables = [SparsePauliOp(Pauli("ZIII")), SparsePauliOp(Pauli("IZII")), SparsePauliOp(Pauli("IIZI"))]
            ...
            if cfg.algorithm_params.method == "classical":
                ...
                net=skolik_arch(n_qubits=cfg.quantum_model_params.n_qubits, 
                        ...
                        g_spsa_param_ratio = cfg.algorithm_params.g_spsa_param_ratio, observables=observables)
                if noisy_flag:
            ...
            train_executer = executer(train_loader, net=net, optimizer=optim, sk_metric=accuracy_score, type="classification", grad_type=cfg.algorithm_params.gradient_type)
            validation_executer = executer(validation_loader, net=net, sk_metric=accuracy_score, type="classification", grad_type=cfg.algorithm_params.gradient_type)
            test_executer = executer(test_loader, net=net, sk_metric=accuracy_score, type="classification", grad_type=cfg.algorithm_params.gradient_type)
                ...
                if cfg.algorithm_params.gradient_type == "Guided-SPSA":
                    ...
                    summary = ", ".join(
                        [  
                            ...
                            f"Val Metric: {np.abs(validation_executer.avg_accuracy): 0.4f}",
                            f"TM: {np.abs(test_executer.avg_accuracy): 0.4f}",
