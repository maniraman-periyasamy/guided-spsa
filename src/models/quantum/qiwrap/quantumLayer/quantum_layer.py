...
# Custom modules
from ...qiwrap.circuitWrapper.analytical_executer import analytical_executer
from ...qiwrap.circuitWrapper.shot_based_executer import shot_based_executer
from ...qiwrap.circuitWrapper.ibmq_executer import ibmq_executer
from ...qiwrap.quantumLayer.torchFunction import HybridFunction
    ...
    def __init__(
        ...
        if type(quantum_weights) == type(None):
            if quantum_weight_initialization == 'random':
                ...
                self.trainable_parameters = np.zeros(len(self.QcN.weight_parameters)).astype(np.float32) #np.random.uniform(low=0.0, high=1e-15, size=len(self.QcN.weight_parameters)).astype(np.float32)
